...
<div class="row table_all">
	...
	<div class="d-flex justify-content-start">
		<h2>${param.boardName }내의 "${param.searchWord }"검색결과</h2>
	</div>
	...
	<c:if test="${pageFirst >5 }">
		<a href="${root }/searchInBoard.do?searchMethod=${param.searchMethod }&boardName=${param.boardName }&searchWord=${param.searchWord}&pageNum=${pageFirst-1}&onePageNum=${param.onePageNum}">이전</a>
	</c:if>
	
	<c:forEach var="i" begin="${ pageFirst}" end="${pageLast }">
		<a href="${root }/searchInBoard.do?searchMethod=${param.searchMethod }&boardName=${param.boardName }&searchWord=${param.searchWord}&pageNum=${i}&onePageNum=${param.onePageNum}">
		<u:pageShape pageNumI="${i }" pageN="${pageNum }">${i }</u:pageShape> 
		</a>
	</c:forEach>
	
	<c:if test="${pageLast < pageEnd}">
		<a href="${root }/searchInBoard.do?searchMethod=${param.searchMethod }&boardName=${param.boardName }&searchWord=${param.searchWord}&pageNum=${pageLast+1}&onePageNum=${param.onePageNum}">다음</a>	
	</c:if>
	<br />
